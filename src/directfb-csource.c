...
static bool load_file( unsigned char **ret_data, unsigned int *ret_len )
{
     struct         stat st;
     FILE          *fp;
     unsigned int   len  = 0;
...
#ifdef HAVE_DIRECTFB
static bool load_image( DFBSurfaceDescription *desc )
{
     ...
     if (!strncmp( signature, "DFIFF", 5 )) {
          ...
          if (!data) {
               ...
               if (fread( data, 1, height * pitch, fp ) != height * pitch) {
                   fprintf( stderr, "Failed to read DFIFF file!\n" );
                   goto out;
               }
          }
     }
...
#else
          fprintf( stderr, "--raw option should be used when building without DirectFB support!\n" );
          return -2;
#endif
     }
